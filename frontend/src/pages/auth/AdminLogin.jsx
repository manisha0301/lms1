  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      ...
      if (finalizeResponse.data.success) {
        localStorage.setItem("adminToken", finalizeResponse.data.token);
        localStorage.setItem("adminUser", JSON.stringify(finalizeResponse.data.user));

        console.log("Admin login successful:", finalizeResponse.data.user);
        navigate("/dash"); // ← CHANGE THIS to your actual admin dashboard route
      } else {
        setOtpError(finalizeResponse.data.message || "Login failed");
      }
    } catch (error) {
      setOtpError(error.response?.data?.message || "Verification / login failed");
